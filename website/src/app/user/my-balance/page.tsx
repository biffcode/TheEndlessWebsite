...
import { FaGem, FaHistory, FaShoppingCart, FaUndo, FaPlus, FaCreditCard, FaDownload, FaCrown, FaRedo, FaExclamationTriangle } from 'react-icons/fa';
...
export default function GemsDetailPage() {
  ...
  const { user, allocateBattlePassGems, updateGems } = useAuth();
  ...
  useEffect(() => {
    if (user) {
      ...
      // Set maximum refund amount (cannot refund more than current balance)
      setMaxRefundAmount(user.gems);
    }
  }, [user]);
  ...
  const handleOpenRefundModal = () => {
    ...
    // Default to refunding 100 gems or max available
    const defaultRefund = Math.min(100, user.gems);
  ...
  const handleRefundAmountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Ensure refund amount is not more than available gems
    const validAmount = Math.min(amount, maxRefundAmount);
    setRefundAmount(validAmount);
  };

  // Process the refund
  const processRefund = async () => {
    if (!user || refundAmount <= 0) return;
    
    setIsRefunding(true);
    
    try {
      // Negative amount for refund (removing gems)
      const success = await updateGems(-refundAmount);
      
      if (success) {
        // Add refund record to the purchase history with negative amount
        const refundRecord: PurchaseRecord = {
          id: Date.now(),
          date: new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' }),
          description: "Gems Refund",
          amount: -refundAmount,
          price: 0.00, // No price for refunds
          status: "Completed",
          receipt: true
        };
        
        // Update the purchase history with the refund record
        setPurchaseHistory(prevHistory => [refundRecord, ...prevHistory]);
        
        // Update the balance summary
        setBalanceSummary(prev => ({
          ...prev,
          purchased: prev.purchased - refundAmount,
        ...
        <div className="mb-6 p-4 rounded-lg bg-green-500/20 border border-green-500/30 text-white flex items-center justify-between">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-sm text-white/70">{refundAmount} gems have been removed from your account.</p>
            </div>
          </div>
          <button 
      ...
      <div className={`${currentSettings.cardBg || "bg-black/30"} backdrop-blur-sm rounded-lg p-6 border border-white/10 mb-8 relative`}>
        ...
        <div className="flex flex-wrap gap-3 mt-6 justify-center">
          ...
          <button 
            ...
            disabled={!user || user.gems <= 0}
            className={`bg-white/10 hover:bg-white/20 text-white px-4 py-2 rounded-md transition-all flex items-center gap-2 ${!user || user.gems <= 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            <FaUndo />
      ...
      {showRefundModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className={`${currentSettings.cardBg || "bg-black/90"} rounded-lg p-6 border border-white/20 max-w-md w-full`}>
            <h3 className="text-xl font-bold text-white mb-2">Refund Gems</h3>
            
            <div className="border-b border-white/10 pb-4 mb-4">
              <p className="text-white/80 text-sm mb-4 flex items-start gap-2">
                <FaExclamationTriangle className="text-yellow-400 mt-1 flex-shrink-0" />
                <span>You can refund unused gems from your account. This action can't be undone. Any refunded gems will be removed from your balance.</span>
              </p>
            </div>
            
            <div className="space-y-4 mb-6">
              <div>
                <label className="text-white/80 text-sm block mb-2">Current Balance</label>
                <div className="flex items-center gap-2 text-white font-semibold">
                  <FaGem className={`h-4 w-4 ${currentSettings.featureHighlight || 'text-amber-400'}`} />
                  <span>{user?.gems || 0} gems</span>
                </div>
              </div>
              
              <div>
                <label htmlFor="refundAmount" className="text-white/80 text-sm block mb-2">Refund Amount</label>
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    id="refundAmount"
                    ...
                    min={1}
                    max={maxRefundAmount}
                    className="bg-black/40 border border-white/20 rounded-md px-3 py-2 text-white w-full focus:outline-none focus:ring-2 focus:ring-white/30"
                  />
                  <span className="text-white whitespace-nowrap flex items-center gap-1">
                    <FaGem className={`h-3 w-3 ${currentSettings.featureHighlight || 'text-amber-400'}`} />
                    gems
                  </span>
                </div>
                <div className="flex justify-between mt-1">
                  <button 
                    onClick={() => setRefundAmount(Math.max(1, Math.min(10, maxRefundAmount)))}
                    className="text-xs text-white/60 hover:text-white"
                  >
                    10
                  </button>
                  <button 
                    onClick={() => setRefundAmount(Math.max(1, Math.min(50, maxRefundAmount)))}
                    className="text-xs text-white/60 hover:text-white"
                  >
                    50
                  </button>
                  <button 
                    onClick={() => setRefundAmount(Math.max(1, Math.min(100, maxRefundAmount)))}
                    className="text-xs text-white/60 hover:text-white"
                  >
                    100
                  </button>
                  <button 
                    onClick={() => setRefundAmount(Math.max(1, Math.min(500, maxRefundAmount)))}
                    className="text-xs text-white/60 hover:text-white"
                  >
                    500
                  </button>
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                disabled={isRefunding || refundAmount <= 0 || refundAmount > (user?.gems || 0)}
                className={`flex-1 py-2 px-4 ${currentSettings.buttonColor} text-white rounded-md flex items-center justify-center gap-2 ${isRefunding || refundAmount <= 0 || refundAmount > (user?.gems || 0) ? 'opacity-50 cursor-not-allowed' : 'hover:brightness-110'}`}
              >
                {isRefunding ? (
                  <>
